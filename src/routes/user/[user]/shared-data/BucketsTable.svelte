<script lang="ts">
	...
	if (defaultSelected.length == 0) {
		defaultSelected = ['name', 'type', 'env', 'team'];
	}
	...
	type SortBy = 'NAME' | 'TYPE' | 'ENV' | 'TEAM';
	...
	function getBucketsTableDataSorted(
		...
		return [...data].sort((a, b) => {
			...
			if (sortedBy === 'NAME') {
				...
				let aSlug = a.team.slug;
				let bSlug = b.team.slug;
				if (aSlug > bSlug) result = 1;
				else if (aSlug < bSlug) result = -1;
			} else if (sortedBy === 'TYPE') {
				if (a.kind > b.kind) result = 1;
				else if (a.kind < b.kind) result = -1;
