...
SELECT
	teams.slug, teams.display_name, teams.last_successful_sync, teams.delete_key_confirmed_at, teams.section_code, teams.is_managed, teams.has_manual_editing,
...
func (q *Queries) ListAllForSearch(ctx context.Context) ([]*ListAllForSearchRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.Slug, &i.SectionCode); err != nil {
			return nil, err
		}
		items = append(items, &i)
	}
