...
func (j *jwtAuth) handler(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		var email string
		if err := tok.Get(j.emailClaim, &email); err != nil {
			j.log.Debugf("failed to get email from claim %q in token", j.emailClaim)
			next.ServeHTTP(w, r)
			return
		}
...
func JWTAuthentication(ctx context.Context, issuer, audience, emailClaim string, log logrus.FieldLogger) (func(next http.Handler) http.Handler, error) {
	...
	if emailClaim == "" {
		return nil, errors.New("emailClaim is required")
	}
