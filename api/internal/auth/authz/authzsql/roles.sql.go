...
const serviceAccountHasAuthorization = `-- name: ServiceAccountHasAuthorization :one
SELECT
	(
		EXISTS (
			SELECT
...
type ServiceAccountHasAuthorizationParams struct {
	ServiceAccountID  uuid.UUID
	AuthorizationName string
}

func (q *Queries) ServiceAccountHasAuthorization(ctx context.Context, arg ServiceAccountHasAuthorizationParams) (bool, error) {
	row := q.db.QueryRow(ctx, serviceAccountHasAuthorization, arg.ServiceAccountID, arg.AuthorizationName)
	var column_1 bool
	err := row.Scan(&column_1)
	return column_1, err
}
