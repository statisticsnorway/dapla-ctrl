...
func (g *GoogleResourceManager) GetOrCreateFolder(ctx context.Context, displayName, parent string) (string, error) {
	...
	createdFolder, err := op.Wait(ctx)
	if err != nil {
		return "", fmt.Errorf("wait for folder creation %q under %q: %w", displayName, parent, err)
	}
...
func (g *GoogleResourceManager) GetOrCreateTagValue(ctx context.Context, tagKeyNamespacedName, teamSlug string) (string, error) {
	...
	createdTagValue, err := op.Wait(ctx)
	if err != nil {
		return "", fmt.Errorf("wait for tag value creation %q under %q: %w", teamSlug, tagKeyNamespacedName, err)
	}
	return createdTagValue.NamespacedName, nil
}
